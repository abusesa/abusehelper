...
For an abuse  or vulnerable service report to be actionable and able to reach the right end recipient, various key elements need to be present.
...
{ "description url": "A description URL is a link to a further description of the the event in question." }
...
{ "status": "Observed status of the malicious resource (phishing URL, dropzone, c&c etc), e.g. online, offline." }
...
```json
{ "type": "The type is one of the most crucial pieces of information for any given abuse or vulnerable service event. The main idea of dynamic typing is to keep our ontology flexible, since we need to evolve with the evolving threatscape present in the data. Furthermore, the value data set for the type attribute should be kept as minimal as possible to avoid a **type explosion**, which in turn dilutes the business value of dynamic typing. In general, we normally have two types of abuse: victims and compromised infrastructure. Vulnerable services in turn relate to misconfigured services available for abuse without a compromise." }
```
...
{ "defacement": "This IOC refers to hacktivism related activity." }
