...
class AtlasSRFService(roomfarm.RoomFarm):
    ...
    def _main(inner, self, global_dedup):
        while True:
            yield inner.sub(atlassrf(global_dedup, self.opener, self.url))
...
def main(name, xmpp_jid, service_room, feed_url, poll_interval=60.0*60.0, 
    ...
    def bot(inner):
        ...
        service = AtlasSRFService(xmpp, feed_url, state_file, poll_interval)
